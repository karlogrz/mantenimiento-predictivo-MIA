        ...
        for i in range(50):  # Menos iteraciones para mejor performance
            ...
            if datos_arduino:
                ...
                if not st.session_state.datos_reales.empty:
                    # LIMPIAR DATOS ANTES DE CREAR EL GRÁFICO - CORRECCIÓN
                    ...
                    # Crear el gráfico con datos limpios
...
with tab2:
    ...
    if 'datos_reales' in st.session_state and len(st.session_state.datos_reales) > 0:
        ...
        # Limpiar datos para el análisis histórico también
        ...
        if "RPM" in datos_limpios.columns and "Temperatura (°C)" in datos_limpios.columns:
            # Gráfico con paleta personalizada
            fig_hist = px.scatter(
                datos_limpios,
                x="RPM",
                y="Temperatura (°C)",
