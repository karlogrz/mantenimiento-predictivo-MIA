            ...
            if datos_arduino:
                ...
                if not st.session_state.datos_reales.empty:
                    fig = px.line(st.session_state.datos_reales, x="Hora", y=variables, 
                                 ...
                                 color_discrete_sequence=PALETA_COLORES)  # ← Paleta personalizada
...
with tab2:
    ...
    if 'datos_reales' in st.session_state and len(st.session_state.datos_reales) > 0:
        ...
        if "RPM" in datos.columns and "Temperatura (°C)" in datos.columns:
            # Gráfico con paleta personalizada - CORREGIDO
            fig_hist = px.scatter(
                datos,
                x="RPM",
                y="Temperatura (°C)",
                ...
                color_continuous_scale=PALETA_COLORES  # ← Paleta personalizada
            )
            # Personalizar aún más el gráfico
            fig_hist.update_layout(
                plot_bgcolor='white',
                paper_bgcolor='white',
                font=dict(color='#006f75'),
                title_font=dict(size=20, color='#015861')
            )
            fig_hist.update_traces(
                marker=dict(size=10, opacity=0.7, line=dict(width=1, color='#00777e'))
            )
            st.plotly_chart(fig_hist, use_container_width=True)
            
            # Gráfico adicional de series temporales
            fig_tiempo = px.line(
                datos,
                x="Hora",
                y=["RPM", "Temperatura (°C)", "Vibración (m/s²)"],
...
with tab3:
    ...
    with col1:
        st.color_picker("Color 1", "#006f75", disabled=True)
    with col2:
        st.color_picker("Color 2", "#00777e", disabled=True)
    with col3:
        st.color_picker("Color 3", "#015861", disabled=True)
    with col4:
        st.color_picker("Color 4", "#016c75", disabled=True)
    with col5:
        st.color_picker("Color 5", "#459ca3", disabled=True)
    with col6:
        st.color_picker("Color 6", "#f7f5f5", disabled=True)
    with col7:
        st.color_picker("Color 7", "#c5d3d5", disabled=True)
    with col8:
        st.color_picker("Color 8", "#abc4c7", disabled=True)
...
with st.expander("ℹ️ Información importante"):
    ...
    **🎨 Paleta de colores:** 
    La aplicación utiliza una paleta de tonos azul-verdes y grises suaves:
    - #006f75 (Azul verde oscuro)
    - #00777e (Azul verde medio)
    - #015861 (Verde azulado oscuro)
    - #016c75 (Azul petróleo)
    - #459ca3 (Turquesa suave)
    - #f7f5f5 (Blanco hueso)
    - #c5d3d5 (Gris azulado claro)
    - #abc4c7 (Gris perla azulado)
    """)