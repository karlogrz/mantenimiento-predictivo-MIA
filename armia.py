...
def analizar_irregularidades_rpm(datos_rpm):
    ...
    if len(datos_rpm) > 0:
        ...
        std_rpm = std_rpm = np.std(datos_rpm) if len(datos_rpm) > 1 else 0
        variacion = (std_rpm / media_rpm) * 100 if media_rpm > 0 else 0  # Variación porcentual
...
if not puertos_disponibles:
    st.sidebar.error("❌ No se detectaron puertos seriales")
...
with tab3:
    ...
    if st.session_state.ser:
        ...
        st.info(f"**Puerto seleccionado:** {puerto_seleccionado}")
