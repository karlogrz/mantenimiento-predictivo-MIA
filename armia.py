...
# ---- Función para encontrar puerto Arduino ----
...
def inicializar_serial():
    puerto_arduino = encontrar_arduino()
    if puerto_arduino:
        try:
            ser = serial.Serial(puerto_arduino, 9600, timeout=1)
            time.sleep(2)  # Esperar a que se establezca la conexión
            # Limpiar buffer serial
            ser.reset_input_buffer()
            return ser
        except Exception as e:
            st.error(f"Error al conectar con Arduino: {e}")
            return None
    return None
...
if 'ser' not in st.session_state:
    st.session_state.ser = inicializar_serial()

if st.session_state.ser is None:
    st.sidebar.error("❌ Arduino no detectado")
    st.sidebar.info("💡 Conecta el Arduino y verifica el puerto COM")
else:
...
if st.sidebar.button("▶️ Iniciar", type="primary", use_container_width=True) and st.session_state.ser:
...
with tab1:
    ...
    if not st.session_state.monitoreo_activo or st.session_state.ser is None:
        ...
        # Gráfico dinámico (streaming de datos reales)
        ...
        for i in range(100):  # 100 iteraciones máximo
...
with tab3:
    ...
    if st.session_state.ser:
        ...
        if st.button("Reconectar Arduino"):
            st.session_state.ser = inicializar_serial()
            st.rerun()
    else:
        ...
        if st.button("Buscar Arduino nuevamente"):
            st.session_state.ser = inicializar_serial()
            st.rerun()
